...
void newProcess(char** Actions,int count) {//Creates a new process
    ...
    if (id == 0) {
        ...
        if (grandson==0){
        char temp[512];
        sprintf(temp,"cat /proc/%d/sched\n",getppid());
        char** array=SplitsTheActions(temp);
        if (execvp(array[0],array)<0){
            printf("exec is failed");
        }
        deleteTheWords(array,2);
        }
        wait(NULL);
        if (execvp(Actions[0], Actions) == -1) {
...
void deleteTheWords(char** a,int t){//free the dynamic assignment
    int m;
    for (m = 0; m<t;m++) {
        free(a[m]);
    }
